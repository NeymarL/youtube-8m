...
JOB_NAME=yt8m_train_MCNN_$(date +%Y%m%d_%H%M%S); gcloud --verbosity=debug ml-engine jobs \
submit training $JOB_NAME \
--package-path=youtube-8m --module-name=youtube-8m.train \
--staging-bucket=$BUCKET_NAME --region=us-east1 \
...
--frame_features=True --model=MeanCNNsModel --feature_names="rgb" \
--feature_sizes="1024" --batch_size=128 \
--train_dir=$BUCKET_NAME/yt8m_train_frame_level_MCNN_model
...
tensorboard --logdir=$BUCKET_NAME/yt8m_train_frame_level_MCNN_NOS_model --port=8080
